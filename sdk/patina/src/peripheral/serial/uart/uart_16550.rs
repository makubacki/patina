...
impl crate::peripheral::serial::SerialIO for Uart16550 {
    ...
    fn read(&mut self) -> u8 {
        // Blocks until a byte is available once the device is constructed. If the address was
        // rejected, there is no hardware to wait on, so a `0` sentinel is returned rather than
        // spinning forever.
        let mut byte = 0u8;
        match self {
            Uart16550::Io(port) => {
...
mod tests {
    ...
    impl FakeMmio {
        ...
        fn new() -> Self {
            let mut regs = [0u8; 8];
            // init() spins on TRANSMITTER_EMPTY and ready_to_send() gates on
            // MSR::CTS, so set both bits to avoid deadlock in the tests.
