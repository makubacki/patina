...
impl TplController for Service<dyn TplServices> {
    fn raise_tpl(&self, tpl: BootServicesTpl) -> BootServicesTpl {
        // `Service<T>` derefs to `&'static T`, so an explicit deref gets to the underlying `dyn TplServices` by
        // name, avoiding ambiguity with the `raise_tpl` implemented here on `Service<dyn TplServices>` itself.
        let services: &dyn TplServices = *Deref::deref(self);
