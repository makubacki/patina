...
pub trait ProtocolServicesExt: ProtocolServices {
    ...
    fn locate_protocol<P: ProtocolInterface>(&self) -> Result<&'static P, ProtocolError> {
        let ptr = self.locate_interface(P::PROTOCOL_GUID.into_inner())?;
    ...
    fn install_protocol<P: ProtocolInterface>(
        ...
        let ptr = ProtocolPtr::from_raw(interface as *const P as *mut c_void).ok_or(ProtocolError::InvalidParameter)?;
        self.install_interface(handle, P::PROTOCOL_GUID.into_inner(), ptr)
    }
    ...
    fn locate_handles_for<P: ProtocolInterface>(&self) -> Result<Vec<Handle>, ProtocolError> {
        self.locate_handles(P::PROTOCOL_GUID.into_inner())
    }
    ...
    fn with_protocol_on<P: ProtocolInterface, R>(
        ...
        let ptr = self.interface_on_handle(handle, P::PROTOCOL_GUID.into_inner())?;
    ...
    /// Locates the first handle with protocol `P` and returns a durable [`ProtocolToken`].
    ///
    ...
    fn locate_token<P: ProtocolInterface>(&self) -> Result<ProtocolToken<P>, ProtocolError> {
        let handle =
            self.locate_handles(P::PROTOCOL_GUID.into_inner())?.into_iter().next().ok_or(ProtocolError::NotFound)?;
    ...
    fn resolve<P: ProtocolInterface>(&self, token: &ProtocolToken<P>) -> Option<&P> {
        let ptr = self.interface_on_handle(token.handle(), P::PROTOCOL_GUID.into_inner()).ok()?;
    ...
    fn on_protocol_installed<P: ProtocolInterface>(
        ...
        self.register_install_notify(P::PROTOCOL_GUID.into_inner(), notify_tpl, Box::new(callback))
    }
...
mod tests {
    ...
    fn test_protocol_services_ext_locate_protocol() {
        ...
        mock.expect_locate_interface().times(1).returning(|guid| {
            assert_eq!(guid, FakeProtocol::PROTOCOL_GUID.into_inner());
            Ok(ProtocolPtr::from_raw(&FAKE_INSTANCE as *const FakeProtocol as *mut c_void).unwrap())
        });
    ...
    fn fake_ptr() -> ProtocolPtr {
        ProtocolPtr::from_raw(&FAKE_INSTANCE as *const FakeProtocol as *mut c_void).unwrap()
    }
