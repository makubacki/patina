...
/// `state` before anything has been published.
const UNINIT: u8 = 0;
/// `state` while a `publish` call is writing `value`.
const WRITING: u8 = 1;
/// `state` once `value` has been fully written and is safe to read.
const DONE: u8 = 2;
...
// SAFETY: All writes to `value` go through `publish`, which is serialized by a `compare_exchange` on `state` so
// that at most one writer ever calls `OnceCell::set`. Reads through `get` only observe `value` after an `Acquire`
// load of `state` returns `DONE`, which is written by the `Release` store at the end of a successful `publish`.
// Observing it establishes a "happens-before" relationship with that specific write. `Service<T>` is `Send` and `Sync`
...
impl<T: ?Sized + 'static> ServiceCell<T> {
    ...
    pub const fn new() -> Self {
        Self { value: OnceCell::new(), state: AtomicU8::new(UNINIT) }
    }
    ...
    pub fn publish(&self, service: Service<T>) -> Result<(), Service<T>> {
        if self.state.compare_exchange(UNINIT, WRITING, Ordering::Acquire, Ordering::Relaxed).is_ok() {
    ...
    pub fn get(&self) -> Option<Service<T>> {
        if self.state.load(Ordering::Acquire) == DONE { self.value.get().copied() } else { None }
    }
...
mod tests {
    ...
    fn test_service_cell_get_returns_none_while_write_in_progress() {
        ...
        cell.state.store(WRITING, Ordering::Release);
        assert!(cell.get().is_none());
        assert!(!cell.is_published());
    }
