...
impl<T: ?Sized + 'static> From<&'static dyn Any> for Service<T> {
    fn from(value: &'static dyn Any) -> Self {
        let s = Self { value: OnceCell::new(), _marker: PhantomData };
    ...
    fn deref(&self) -> &Self::Target {
        if let Some(service) = self.value.get() {
            if let Some(service) = service.downcast_ref() {
                ...
                // Both construction paths (mock, from) guarantee that the downcast will succeed. unreachable! is
                // used here instead of unreachable_unchecked to avoid UB if this invariant is ever violated (e.g.,
                // via the public From<&'static dyn Any> impl with a mismatched type).
                //
            ...
            // Every construction path (mock, from) sets a value before returning, so this is unreachable. We use
            // unreachable! here instead of panic! as this provides compiler hints to the optimizer.
