...
use patina::{
    ...
    error::EfiError,
    performance::{guid::PERFORMANCE_PROTOCOL_GUID, measurement::PerformanceProperty},
};
...
impl PropertyPublisher {
    ...
    fn entry_point(
        ...
        config_table.install_configuration_table(PERFORMANCE_PROTOCOL_GUID.into_inner(), property)?;
...
mod tests {
    ...
    use patina::component::service::{
        performance::MockPerformanceManager, uefi_services::config_table::MockConfigurationTableServices,
    };
    ...
    fn test_property_publisher_entry_point_installs_table() {
        ...
        config_table
            .expect_install_table()
            .once()
            .withf(|guid, _table| {
                assert_eq!(guid, &PERFORMANCE_PROTOCOL_GUID.into_inner());
                true
            })
            .returning(|_, _| Ok(()));
