...
impl PropertyPublisher {
    ...
    /// The DXE Core only publishes [`PerformanceManager`] when performance measurement is enabled, so this
    /// gates its dispatch on the service to ensure the table is only published when performance measurement
    /// and broader performance services are enabled.
    fn entry_point(
        self,
