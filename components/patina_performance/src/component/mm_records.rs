...
impl MmRecordCollector {
    ...
    /// Requires [`MmCommunication`] as a dependency, so the dispatcher itself skips this component on
    /// platforms without a MM communication region.
    fn entry_point(
        self,
        ...
        // Note: The event group may fire more than once. Only the first signal should collect the records.
