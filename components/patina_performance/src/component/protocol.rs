...
//!
//! Protocol symbols are intentionally not public in this file to encapsulate their details within the performance
//! component and service-based access patterns.
//!
//! ## License
//!
...
const EDKII_PERFORMANCE_MEASUREMENT_PROTOCOL_GUID: BinaryGuid =
...
type CreateMeasurementUefi = unsafe extern "efiapi" fn(
...
#[repr(C)]
struct EdkiiPerformanceMeasurementProtocol {
    /// Function to create performance record with event description and a timestamp.
    create_performance_measurement: CreateMeasurementUefi,
}
...
/// The protocol instance installed for C drivers. It has a single function with no context parameter, so the
/// struct does not carry state of its own, see [`PERF_SERVICE`] for how [`create_performance_measurement_efiapi`]
...
/// context parameter to carry the service through directly.
static PERF_SERVICE: ServiceCell<dyn PerformanceManager> = ServiceCell::new();
